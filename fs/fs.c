...
diskaddr(u_int blockno)
{
	if (blockno >= DISKMAX/BY2BLK)
		user_panic("diskaddr, illegal blockno %d\n", blockno);
...
map_block(u_int blockno)
{
	...
	int va = diskaddr(blockno);
	int r = syscall_mem_alloc(0, va, PTE_V | PTE_R);
	if (r != 0)
		user_panic("map_block fail %d\n", blockno);
	return 0;
}
...
unmap_block(u_int blockno)
{
	...
	int va = diskaddr(blockno);
	r = syscall_mem_unmap(0, va);
	if (r != 0)
		user_panic("unmap_block fail %d\n", blockno);
...
free_block(u_int blockno)
{
	...
	if (blockno == 0)
		user_panic("free block number 0\n");
