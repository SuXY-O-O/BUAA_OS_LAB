...
{
	msyscall(SYS_putchar, 1, (int)ch, 0, 0, 0);
}
...
{
	return msyscall(SYS_env_destroy, 1, envid, 0, 0, 0);
}
int
syscall_set_pgfault_handler(u_int envid, void (*func)(void), u_int xstacktop)
{
	return msyscall(SYS_set_pgfault_handler, 3, envid, (int)func, xstacktop, 0);
}

int
syscall_mem_alloc(u_int envid, u_int va, u_int perm)
{
	return msyscall(SYS_mem_alloc, 3, envid, va, perm, 0);
}
...
{
	return msyscall(SYS_mem_unmap, 2, envid, va, 0, 0);
}

int
syscall_set_env_status(u_int envid, u_int status)
{
	return msyscall(SYS_set_env_status, 2, envid, status, 0, 0);
}

int
syscall_set_trapframe(u_int envid, struct Trapframe *tf)
{
	return msyscall(SYS_set_trapframe, 2, envid, (int)tf, 0, 0);
}

void
syscall_panic(char *msg)
{
	msyscall(SYS_panic, 1, (int)msg, 0, 0, 0);
}
...
{
	msyscall(SYS_ipc_recv, 1, dstva, 0, 0, 0);
}
...
{
	msyscall(SYS_smp, 8, a, b, c, d, e, f, g, h);
}
